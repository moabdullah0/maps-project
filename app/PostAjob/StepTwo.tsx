...
const StepTwo = ({ onNext, onBack }: { onNext: (data: StepTwoFormData) => void, onBack: () => void }) => {
  const { register, handleSubmit, setValue, formState: { errors } } = useForm<StepTwoFormData>({
  ...
  const handleEditorChange = (content: string) => {
    ...
    localStorage.setItem('stepTwoData', JSON.stringify({ ...currentData, description: content }));
  };

  const handleSkillsChange = (selectedOptions: any) => {
    setValue('skills', selectedOptions);
    const currentData = JSON.parse(localStorage.getItem('stepTwoData') || '{}');
    localStorage.setItem('stepTwoData', JSON.stringify({ ...currentData, skills: selectedOptions }));
  };
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="mt-4">
        ...
        <TinyMCEEditor value={description} onEditorChange={handleEditorChange} />
        {errors.description && (
